...
route.get("/", async (req, res) => {
     ...
     try{
          ...
          const termini = await IzabranaOpcija.findAll();
 ...
 route.get("/:id", async (req, res) => {
    try{
          ...
          const termin = await IzabranaOpcija.findByPk(req.params.id);
 ...
 route.post("/", authAdminToken, async (req, res) => {
    try{
         ...
         const novi = await IzabranaOpcija.create(req.body);
 ...
 route.put("/:id", authAdminToken, async (req, res) => {
    try{
         console.log("izmena podataka termina čiji je id ="+req.params.id+" a podaci su u req.body");
          const jelo = await IzabranaOpcija.findByPk(req.params.id);
